...
- [ ] 플레이어 선택 뷰를 출력한다.
    - [ ] 존재하는 플레이어 이름을 출력한다.
    - [ ] `player add 이름`: 플레이어 추가
        - [ ] player 이름이 중복되면 예외를 발생시킨다.
    - [ ] `player 이름`: 플레이어 선택
        - [ ] 존재하지 않는 player 이름을 입력하면 예외를 발생시킨다.
- [ ] 게임방 선택 뷰를 출력한다.
    - [ ] 존재하는 게임방 이름을 출력한다.
    - [ ] `room add 이름`: 게임방 추가
        - [ ] room 이름이 중복되면 예외를 발생시킨다.
    - [ ] `room 이름`: 게임방 선택
        - [ ] 존재하지 않는 room 이름을 입력하면 예외를 발생시킨다.
    - [ ] room을 선택하면 room에 해당하는 piece 정보를 조회한다.
- [x] 명령을 입력한다.
    - 명령 종류
        - [x] start: 게임 시작
        - [x] end: 게임 종료
...
- [ ] 플레이어는 구분자(PK)를 가진다.
- [ ] 플레이어는 중복되지 않는 이름을 가진다.
    - [ ] player의 이름은 1자 ~ 4자가 아니면 예외를 발생시킨다.
- [ ] 플레이어는 이름을 알려줘야한다.

### 게임방(Room)

- [ ] 게임방은 구분자(PK)를 가진다.
- [ ] 게임방은 중복되지 않는 이름을 가진다.
- [ ] 게임방의 이름은 1자 ~ 4자가 아니면 예외를 발생시킨다.
